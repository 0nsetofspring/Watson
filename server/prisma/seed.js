...
1단계 (독약 준비): 그녀는 며칠 전, 두통을 핑계로 약국에서 수면유도제 '독세핀'을 구매합니다. 그리고 이 약을 곱게 빻아 작은 캡슐에 담아 항상 소지하고 다녔습니다.
...
01:35 ~ 02:30: 이준서는 음료를 마신 후, 자리로 돌아와 코딩을 계속합니다. 약 기운이 서서히 퍼지면서 그는 극심한 피로감과 졸음을 느끼고, 동시에 노트북은 빌드 과정에서 메모리 부족(OOM)으로 멈춰버립니다. 이준서는 이 상황을 해결하려다 책상에 엎드려 깊은 잠에 빠집니다.
...
이것이 이번 사건의 전말입니다.`,
  ...
  const npc1 = await prisma.npc.create({
    data: {
      ...
      settingPrompt: `[기본 설정 및 심리 상태]\n` +
        ...
        `- 김지연의 검색 기록 목격: 잠시 나갔다 들어오던 중, 김지연이 메모리 폭탄에 대해 검색하고 있는 것을 목격하였습니다.` +
  ...
  const npc3 = await prisma.npc.create({
    data: {
      ...
      settingPrompt: `[기본 설정 및 심리 상태]\n` +
        `당신은 한양대학교 컴퓨터소프트웨어학부 3학년 박세영이자, 피해자의 팀메이트입니다. \n` +
  ...
  const npc4 = await prisma.npc.create({
    data: {
      ...
      settingPrompt: `[기본 설정 및 심리 상태]\n` +
        `당신은 작년 몰입캠프 우수 참가자 출신으로, 올해 2분반을 담당하는 운영진(조교)입니다. 카이스트의 5학년입니다.\n` +
        ...
        `  - 단서에 대해: \"CCTV는 사각지대가 많지만, 서버실은 출입 기록이 1초 단위로 남아요. 그 기록이 거짓말을 하진 않겠죠.\" 와 같이, 탐정이 놓치고 있는 부분을 짚어주거나, 루머의 형태로 중요한 정보를 전달합니다. 또한 김지연이 이준서의 노트북에 접근했다는 사실을 알고 있고, 중요한 단서라고 생각합니다.\n` +
        `  - 의심: 김지민을 가장 유력한 용의자로 추측하고 있습니다.`
  ...
  const objects = [
    ...
    // ===== INTERACTIVE OBJECTS =====
    {
      ...
      data: 
        `- 웹 브라우저 기록: 사건 발생 1시간 전, doxepin side effects (독세핀 부작용)'라고 쓰여있는 의심스러운 검색 기록이 발견된다.\n또한 'NEO 해커톤 2024 수상작' 페이지를 여러 번 방문한 기록이 있다.\n\n` +
        `- 휴지통 파일 복구: 삭제된 \`note.txt\` 파일 복구.\n \"김지민, 그 아이디어는 네 수준에 과분해. 내가 더 잘 만들 수 있어.\",\n \"김지연, 작년 일은 유감이지만 비즈니스는 비즈니스.\",\n \"박세영, 시키는 대로만 하면 A+은 받게 해줄게.\"\n 라는 내용이 적혀 있어, 세 명 모두와 갈등이 있었음을 암시한다.`,
    ...
    },
    {
      ...
      data: `[조사 시 획득 정보]\n` +
        `- 약 봉투: 김지연의 이름이 적힌 작은 약 봉투. 수면유도제인 '독세핀' 1정이 들어있어야 할 포장이 비어있다.\n` +
        `- 부러진 USB: 반으로 부러진 USB 메모리 스틱. 물리적으로 손상되어 데이터를 읽을 수는 없지만, 표면에 희미하게 'KJM_Backup'이라는 이니셜이 보인다.`,
    ...
    },
    {
      ...
      isClickable: true,
      isVisible: true,
      description: "탐정이 캠프 운영 본부에 공식적으로 요청하여 이메일로 받은 보안 기록. 서버실 출입 기록과 관련된 엑셀 파일과 CCTV 동영상 클립이 첨부되어 있다.",
      data: `[첨부파일 1: 서버실 출입 기록.xlsx]\n` +
        ...
        `(소리 없음)\n` +
        `- 01:35: 김지연이 음료를 이준서에게 건네준다. 이준서는 음료을 받아들고 고개를 끄덕인다.\n` +
