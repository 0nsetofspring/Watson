...
이것이 이번 사건의 전말입니다.`,
  ...
  const npc4 = await prisma.npc.create({
    data: {
      ...
      settingPrompt: `[기본 설정 및 심리 상태]\n` +
        ...
        `  - 단서에 대해: \"CCTV는 사각지대가 많지만, 서버실은 출입 기록이 1초 단위로 남아요. 그 기록이 거짓말을 하진 않겠죠.\" 와 같이, 탐정이 놓치고 있는 부분을 짚어주거나, 루머의 형태로 중요한 정보를 전달합니다. 또한 김지연이 이준서의 노트북에 접근했다는 사실을을 알고 있습니다.\n` +
  ...
  const objects = [
    ...
    },
    {
      ...
      data: `[조사 시 획득 정보]\n` +
        `- 약 봉투: 김지연의 이름이 적힌 작은 약 봉투. 처방전 없이 구매 가능한 강력한 수면유도제 '독세핀' 1정이 들어있어야 할 포장이 비어있다.\n` +
        `- 부러진 USB: 반으로 부러진 USB 메모리 스틱. 물리적으로 손상되어 데이터를 읽을 수는 없지만, 표면에 희미하게 'JM(지민)_Backup'이라는 이니셜이 보인다.`,
