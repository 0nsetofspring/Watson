...
async function main() {
  ...
  await prisma.interactiveObject.createMany({
    data: [
      {
        roomId: room102.id, name: "이준서의 노트북", type: "evidence", x: "65%", y: "50%", icon: "💻",
      ...
      {
        roomId: hallway.id, name: "102호 앞 쓰레기통", type: "evidence", x: "80%", y: "75%", icon: "🗑️",
      ...
      {
        roomId: room102.id, name: "102호 칠판", type: "clue", x: "20%", y: "30%", icon: "칠판",
      ...
      {
        roomId: room102.id, name: "캠프 운영 본부 이메일", type: "evidence", x: "50%", y: "10%", icon: "문서",
      ...
      {
        roomId: hallway.id, name: "박세영의 다이어리", type: "evidence", x: "15%", y: "80%", icon: "다이어리",
      ...
      {
        roomId: lounge.id, name: "서버실 마스터 키", type: "key", x: "90%", y: "15%", icon: "🔑",
        description: "라운지 구석, 여러 개의 분실물 중 하나인 낡은 카드키. '서버 관리실'이라고 적혀있다.",
        data: "이 키를 사용하면 '비밀의 방: 서버 관리실'에 들어갈 수 있다."
      },
      {
        roomId: serverRoom.id, name: "서버실 내부 랜선", type: "evidence", x: "30%", y: "60%", icon: "纜線",
